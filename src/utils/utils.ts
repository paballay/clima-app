...
/**
 * Transform current date [new Date()] to 'AAAA/MM/DD'
 * @returns string 'AAAA/MM/DD'
 */
export function formatCurrentDateToString(): string {
 ...
 *
 * @param date string
 * @returns number: format UnixTime
 */
export function dateToUnixTime(date: string): number {
  const dateFormat = date.replaceAll('-', '/');

  return Math.floor(new Date(dateFormat).getTime() / 1000);
}
